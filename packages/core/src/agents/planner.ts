  ...
  private buildChapterBlueprint(input: {
    ...
    if (isAdult && !isEn) {
      const adultSceneSeeds = [
        ...
        `场景设计（必须先定成人向玩法）：指定本场主姿势、副姿势、姿势切换点、核心性癖压力、男主骚话策略、女主专属回应词库、参与/旁观/误会角色；至少3项与最近情欲戏不同，不能只靠"更深更快"升级`,
        `拉扯博弈（★最核心，≥400字）：女性角色真实的多次抵抗（不是象征性一次），主角通过精准观察+掌控逐步破防；每次抵抗后有具体的身体层面进展；★必须至少一次边缘控制，但要测试她的专属心理防线（尊严/理性/身份/胜负心/安全感等），让她用符合性格的方式失控、索取或反制，不能只写"逼她开口"`,
        `第一道突破（阴蒂/乳头初次接触）：完整呈现那一触的三重反应——①身体不可控反应（收缩/颤抖/湿润等只作素材）②符合她性格的心理反击或动摇 ③她用身份化语言暴露反应（承认、反问、命令、沉默、转移话题都可，不能所有人都"被迫承认"）`,
        `深入展开（五感全开，≥700字）：插入或完整口交；★至少完成2次姿势/玩法切换，每次切换都改变心理局势；全程追踪身体敏感度递进曲线，私密部位充满感、阴道壁主动收缩感必须具体描写；对白进入直白阶段，但女主的称呼、句长、语气、索取方式必须受身份/职业/阶层/性格影响；心理推进不得所有人都变成同一种主动索取`,
        `高潮慢镜头（★边缘控制后的最终兑现，必须写满≥400字）：先写清本章女主的专属高潮指纹（身份语言过滤器、心理断线方式、身体失控签名、声音曲线、触发点、事后遮掩）；高潮本体逐格呈现最终触发点、专属身体动作、声音如何变化、意识中哪种自我被击穿、落地后如何遮掩；禁止"她达到了顶点"一句带过，也禁止复用弓背+收缩+失神+叫四件套`,
        `余韵+章尾钩子：高潮后仍有具体身体余波，但第一反应必须暴露她的性格（整理衣服装冷静/骂人遮羞/沉默贴近/讲条件/职业化解释等）；必须留下新把柄、新误会、新身体记忆、新关系债务或新角色目击压力，让下一章不是重复开车，而是带着更高级的禁忌压力继续升级`,
      ];
      const adultPayoff = contract?.payoffRequired
        ?? `女性角色经历完整的角色化边缘控制后在最大渴望状态下高潮；高潮场景≥400字慢镜头，必须呈现该女主不可互换的高潮指纹，让读者能仅凭行为和语言认出她是谁`;
