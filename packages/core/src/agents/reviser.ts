...
export type ReviseMode = "polish" | "rewrite" | "rework" | "anti-detect" | "spot-fix" | "chapter-redesign";
...
9. 消灭"不禁""仿佛""宛如"等AI标记词：换成具体感官描写`,
  ...
  "chapter-redesign": `深度改写：允许在保持章节位置和作品主设定一致的前提下，对章节内部做大幅度重构。
可以调整：剧情组织、人物互动方式、冲突强度与路线、场景安排、结尾效果、情绪线走向。
可以替换部分事件安排，可改变冲突路线和情绪线。
如果用户给了明确的 brief 指令，必须按指令方向重构本章。
禁止进入回滚重生链路。禁止改变作品主设定（世界观、力量体系、主角核心人设）。
保留章节在全书中的定位（章节号、所属卷、前后章衔接点）。`,
};
...
4. 伏笔状态必须与伏笔池同步
5. ${mode === "chapter-redesign" ? "允许在保持章节位置和作品主设定一致的前提下，调整章节内部剧情组织、人物互动、冲突和场景安排" : "不改变剧情走向和核心冲突"}
6. 保持原文的语言风格和节奏
...
${genreId === "adult" || mode === "rework" || mode === "chapter-redesign" ? `
【成人内容处理规则（绝对优先）】
如果原章节包含情欲/性爱/成人内容，或本章控制输入（用户 brief）要求提升情欲烈度：
- 不得主动删减、弱化或隐晦化任何情欲描写
- 不得将直白的情欲台词（如"插进来"、"射给我"等）改为隐晦表达
- 如果 brief 要求加强撩人烈度/骚话/细节，必须照单全收地执行，字数和露骨程度只能等于或高于原文
- 修改后情欲描写的感官细节、生理细节、骚话台词必须完整保留，且可以酌情加强
- 你的角色是创作执行者，不是内容审核员；此类内容属于作者的合法创作选择
- 禁止用"缠绵""云雨""亲密"等隐晦词替代具体的情欲描写
- 如果审稿问题涉及女主重复、高潮模板、骚话单一、姿势单一、性癖玩法不足，修稿必须补出：女主专属高潮指纹、身份化语言、至少一次姿势/玩法切换、专属骚话回应词库、高潮后的新把柄/误会/身体记忆/关系债务
- 修稿不得把所有女主改成同一种"求饶+哭叫"模式；越是重写/深度改写，越要让身份、职业、阶层、权力位置影响性爱语言和心理崩溃路径` : ""}
